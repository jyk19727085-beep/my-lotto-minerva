...
# 1. 페이지 기본 설정
st.set_page_config(
    page_title="미네르바 1225회차 마스터 V7.0", 
    layout="wide",
...
# ==========================================
# [하단부] 가중치 제어 패널 (1224회차 오차 보정 반영)
# ==========================================
with settings_area:
    st.markdown("<br><br>", unsafe_allow_html=True)
    with st.expander("⚙️ 9대 퀀트 가설 가중치 (1225회차 리밸런싱 완료)", expanded=False):
        hypotheses = ["최근 빈도", "장기 미출", "동반 출현(↑)", "홀짝 균형", "공간 패턴(↑)", "구간 쏠림", "10회 갭", "수분포 매물", "기초 체력"]
        raw_weights = []
        cols = st.columns(3)
        # 1224회차 배수/이월 현상 분석을 통해 도출된 새로운 황금 비율
        def_vals = [45, 55, 60, 50, 65, 45, 50, 55, 60]
        
        for i, hyp in enumerate(hypotheses):
            with cols[i % 3]:
                w = st.slider(f"{hyp}", 0, 100, def_vals[i], key=f"v70_w_{i}")
...
# ==========================================
with header_area:
    st.title("🏆 프리미엄 6/45 마스터 (V7.0 1225회차)")
...
if remaining > 0:
    with button_area:
        if st.button("🚀 1225회차 정밀 스캐닝 및 1세트 추출 (15초)", use_container_width=True, type="primary"):
            ...
            with display_area:
                ...
                progress_bar.empty()
                status_text.markdown("<p style='text-align:center; font-size:1.5rem; font-weight:900; color:#4ade80;'>✅ 분석 완료! 1225회차 최적 수렴 데이터 도출</p>", unsafe_allow_html=True)
                time.sleep(0.5)
                ...
                st.markdown(f"<h2 style='text-align:center; color:#FFD700;'>🎯 1225회차 마스터 1세트 (조화 점수: {harmony:.1f}%)</h2>", unsafe_allow_html=True)
                ...
                st.balloons()
                st.success("🎉 주간 3회 한정, 1224회차의 오차를 완벽히 보정한 조합이 생성되었습니다.")
    ...
    st.info("💡 새로고침을 하거나 브라우저를 껐다 켜면 횟수가 초기화될 수 있습니다. 실제 주간 관리는 Daniel님의 신중한 판단에 맡깁니다.")
